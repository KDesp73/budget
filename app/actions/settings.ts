...
export async function saveCategoryBudgets(budgets: Record<string, number>) {
  await verifySession();

  await db.execute(
    "INSERT INTO settings (key, value) VALUES ('category_budgets', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value",
